...
export default memo(function PositionView({
  ...
  // ─── 競合選択 ────────────────────────────────────
  const firstCompetitorId = useMemo(() => {
    const comp = apps.find(a => !(a.isMain || a.id === 'target'))
    return comp?.id ?? null
  }, [apps])
  const [selectedCompetitor, setSelectedCompetitor] = useState(firstCompetitorId)
      ...
      <div className="panel">
        ...
        <div className="panel-body">
          ...
          {appSummaries.filter(a => !a.isMain).map(app => {
            const isSelected = (selectedCompetitor ?? firstCompetitorId) === app.id
            const compData = apps.find(a => a.id === app.id)
            return (
              <div key={app.id}
                style={{ padding: '5px 8px', borderRadius: 6, border: `1px solid ${isSelected ? `${app.color}44` : '#21262d'}`, background: isSelected ? `${app.color}08` : 'transparent', cursor: 'pointer', marginBottom: 4 }}
                onClick={() => setSelectedCompetitor(app.id)}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {isSelected && compData && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 6, paddingTop: 5, borderTop: `1px solid ${app.color}22` }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 10, color: '#f85149', marginBottom: 3, fontWeight: 600 }}>不満</div>
                      {(compData.top_complaints || []).map((c, i) => (
                        <div key={i} style={{ fontSize: 10, color: '#e6edf3', padding: '2px 0', borderBottom: '1px solid #21262d', lineHeight: 1.4 }}>{c}</div>
                      ))}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 10, color: '#56d364', marginBottom: 3, fontWeight: 600 }}>好評</div>
                      {(compData.top_praises || []).map((p, i) => (
