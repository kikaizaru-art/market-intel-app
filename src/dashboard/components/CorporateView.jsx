    ...
    <div className="panel">
      ...
      <div className="panel-body">
        ...
        <div className="fundamental-tabs" style={{ marginBottom: 8 }}>
          {TABS.map(t => (<button key={t.key} className={`fundamental-tab corporate-tab ${tab === t.key ? 'active' : ''}`} onClick={() => setTab(t.key)}>{t.label}</button>))}
        </div>

        {tab === 'financial' && latestQ && prevQ && (
          <>
            <div style={{ fontSize: 10, color: '#6e7681', marginBottom: 4 }}>{company.name} — {company.segment} 四半期推移 (億円)</div>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
              ...
              <div className="stat-card"><div style={{ fontSize: 10, color: '#6e7681' }}>営業利益率</div><div style={{ fontSize: 14, fontWeight: 700, color: '#e3b341' }}>{latestQ.op_margin}%</div></div>
              <div className="stat-card"><div style={{ fontSize: 10, color: '#6e7681' }}>上場</div><div style={{ fontSize: 12, fontWeight: 600, color: '#8b949e' }}>{company.listed ? company.ticker : '非上場'}</div></div>
            </div>
          </>
        )}
        ...
        {tab === 'pipeline' && (
          <>
            <div style={{ fontSize: 10, color: '#6e7681', marginBottom: 6 }}>{company.name} — タイトル一覧 ({company.titles.length}本)</div>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
              <div className="stat-card"><div style={{ fontSize: 10, color: '#6e7681' }}>運営中</div><div style={{ fontSize: 14, fontWeight: 700, color: '#56d364' }}>{company.titles.filter(t => t.status === '運営中').length}本</div></div>
              <div className="stat-card"><div style={{ fontSize: 10, color: '#6e7681' }}>開発中</div><div style={{ fontSize: 14, fontWeight: 700, color: '#388bfd' }}>{company.titles.filter(t => t.status === '開発中').length}本</div></div>
            </div>
          </>
        )}
        ...
        {tab === 'structure' && (
          <>
            <div style={{ fontSize: 10, color: '#6e7681', marginBottom: 8 }}>{company.name} — 開発体制</div>
            <div style={{ fontSize: 10, color: '#6e7681', marginBottom: 4 }}>従業員数比較</div>
            <div className="structure-bars">
              {companies.map((c, i) => {
                const max = Math.max(...companies.map(co => co.headcount))
                const pct = max > 0 ? c.headcount / max * 100 : 0
                return (
      ...
      <div className="panel-footer">generated data — 実データ: 決算短信 / IR / 採用ページ</div>
    </div>
  )
})
